...
// CalcAndUpdateCellValuesDependencyAware calculates and updates multiple cell values with dependency-aware optimization.
// Similar to CalcCellValuesDependencyAware but also writes the calculated values back to cells and triggers OnCellCalculated callback.
//
// It uses a two-phase strategy:
// 1. Phase 1 (Warmup): Sequentially calculates and updates a small batch of formulas to warm up caches
// 2. Phase 2 (Concurrent): Calculates and updates remaining formulas using multiple goroutines
//
// This is most effective when:
//   - You have many formulas (> 1000 cells)
//   - Formulas share dependencies (e.g., multiple VLOOKUP referencing same table)
//   - You need to update cell values and trigger callbacks
//   - You want maximum performance with side effects
//
// The OnCellCalculated callback (if set) will be triggered for each cell whose value changes.
func (f *File) CalcAndUpdateCellValuesDependencyAware(sheet string, cells []string, options CalcCellValuesDependencyAwareOptions) (map[string]string, error) {
	if len(cells) == 0 {
		return make(map[string]string), nil
	}
	...
	// Pre-warm worksheet cache
	_, _ = f.workSheetReader(sheet)

	// Pre-warm commonly referenced sheets
	sheetList := f.GetSheetList()
	for i := 0; i < len(sheetList) && i < 3; i++ {
		if sheetList[i] != sheet {
			_, _ = f.workSheetReader(sheetList[i])
	...
	if options.EnableDebug {
		warmupElapsed := time.Since(warmupStart)
		cacheCount := f.rangeCache.Len()
		fmt.Printf("Phase 1 completed: %d cells calculated and updated, rangeCache entries: %d, elapsed: %v\n",
			len(warmupList), cacheCount, warmupElapsed)
	}
